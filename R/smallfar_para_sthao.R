...
#' 
#' This function returns an estimation of theta as defined in Naveau et al (2018) that is used 
...
#' elements:
#' \describe{
#'   \item{theta_hat}{the estimation of theta}
...
#' 
#' This function returns an estimation of the far, the fraction of attributable risk for records,
...
#' J. Clim., 2018.}, \url{https://doi.org/10.1175/JCLI-D-16-0752.1}
#' 
#' @param theta_hat a vector of the estimated theta for different return period \code{rp}.
#' The estimation of theta is made assuming W ~ exp(theta). 
#' This vector is provided by the function \code{\link{estim_theta.wexp}}
#' @param sigma_theta_hat a vector with the estimated standard devitation of \code{theta_hat}
#' this vector is provided by the function \code{\link{estim_theta.wexp}}
#' @param rp the return periods for which \code{theta_hat} and  \code{signa_theta_hat}
#' are provided.
#' 
...
plot.farrfit <-function(x, ...){
  ...
  lines(rp, farr, col="black", lwd = 1, lty = 1)
  #title("far(r)")
  invisible(NULL)
}
...
hist.thetafit_wexp <- function(x, ...){
  ...
  hist(x$W,
       ...
       breaks = max(9, length(x$W)/10), 
       col="lightgray"
  )
  grid(lwd = 3)
  box()
  xx <- seq(from = 0, to = max(x$W), length=200)
...
#' the bootstrap estimation of theta assuming W ~ exp(theta). 
#' 
#' This function returns bootstrap estimations of theta as defined in Naveau et al (2018) that is used 
#' to estimate the far, the fraction of attributable risk for records. This estimation is made assuming that W = - log(G(Z)) follows an 
#' exponentional distribution: W ~ exp(theta). G denotes the Cumulative Distribution Function of
...
#' elements:
#' \describe{
#'   \item{theta_boot}{a vector with the estimations of theta for each 
#'   bootstrap sample}
...
#' for the far computed empirically from the bootstrap estimations of the far
#' for different return periods \code{rp}
#' 
